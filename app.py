...
idx = ma_tram_list.index(ma_tram)
user_data = df_csdl.iloc[idx].to_dict()
...
# -------------------- AUTO FIELDS --------------------
lc = str(user_data.get("Loai_cot", "")).strip().lower()
user_data["Danh_gia_cot"] = "Đạt" if lc == "cột dây co" else "Không đánh giá"
...
def do_rotate(idx, angle):
    ...
    if key in st.session_state.images:
        rotated = st.session_state.images[key].rotate(angle, expand=True)
...
for i, label in enumerate(labels, start=1):
    ...
    if key in st.session_state.images:
        ...
        with col1: st.image(st.session_state.images[key], width=450)
        with col2: st.button("⟲", key=f"L{i}", on_click=do_rotate, args=(i, 90))
        with col3: st.button("⟳", key=f"R{i}", on_click=do_rotate, args=(i, -90))

    st.markdown("---")

# -------------------- PLACEHOLDER AUTO-SCAN --------------------
def extract_placeholders_bytes(docx_bytes):
...
if st.button("📄 Tạo & Tải biên bản"):
    ...
    try:
        with st.spinner("Đang tạo biên bản…"):
            ...
            placeholders = extract_placeholders_bytes(docx_bytes)
            ...
            for k, v in user_data.items():
                ...
                normalized[k2] = "" if v is None else str(v)

            # REPLACE TEXT
            for ph in placeholders:
                ph_norm = ph.lower().replace("_","")
                value = normalized.get(ph_norm, "")

                # Replace both types
                for form in [f"${ph}", f"${{{ph}}}"]:
                    ds.replace_text(doc, form, value)
            ...
            for i in range(1, 9):
                ...
                if key in st.session_state.images_bytes:
                    ...
                    for form in [f"$Anh{i}", f"${{Anh{i}}}", f"$anh{i}", f"${{anh{i}}}"]:
                        ds.insert_image(doc, form, img_bytes, 12)

            # SAVE DOCX
            out_bytes = ds.save_docx(doc)

            name = f"BBNT_{ma_tram}_{thang}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.docx"
